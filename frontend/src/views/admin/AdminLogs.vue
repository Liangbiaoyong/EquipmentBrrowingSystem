  ...
  <div class="logs"><h2>操作日志</h2>
    <el-card>
      <el-row :gutter="10" style="margin-bottom:15px">
        <el-col :span="5"><el-input v-model="uname" placeholder="搜索用户名" clearable @keyup.enter="load"/></el-col>
        <el-col :span="3"><el-select v-model="ustatus" placeholder="状态" clearable @change="load"><el-option label="成功" :value="1"/><el-option label="失败" :value="0"/></el-select></el-col>
        <el-col :span="3"><el-button type="primary" @click="load">搜索</el-button></el-col>
        <el-col :span="13" style="display:flex;justify-content:flex-end;gap:8px">
          <el-button @click="doExport('csv')">导出 CSV</el-button>
          <el-button type="primary" @click="doExport('xlsx')">导出 Excel</el-button>
        </el-col>
      </el-row>
      <el-table :data="list" v-loading="loading" stripe size="small">
        ...
        <el-table-column prop="duration" label="耗时" width="80"><template #default="{row}">{{ row.duration }}ms</template></el-table-column>
        <el-table-column label="状态" width="70"><template #default="{row}"><el-tag :type="row.status===1?'success':'danger'" size="small">{{ row.status===1?'成功':'失败' }}</el-tag></template></el-table-column>
