<template>
  <div class="my"><h2>我的借用</h2>
    <el-card>
      <el-tabs v-model="activeTab" @tab-change="load"><el-tab-pane label="全部" name=""/><el-tab-pane label="待审批" name="PENDING_APPROVAL"/><el-tab-pane label="已通过" name="APPROVED"/><el-tab-pane label="借用中" name="BORROWING"/><el-tab-pane label="已归还" name="RETURNED"/><el-tab-pane label="已驳回" name="REJECTED"/></el-tabs>
      <el-table :data="list" v-loading="loading" stripe>
        <el-table-column label="借用单ID" width="80" prop="id"/><el-table-column label="设备ID" width="80" prop="deviceId"/><el-table-column label="开始时间" width="170"><template #default="{row}">{{ fmt(row.startTime) }}</template></el-table-column><el-table-column label="结束时间" width="170"><template #default="{row}">{{ fmt(row.endTime) }}</template></el-table-column><el-table-column label="状态" width="110"><template #default="{row}"><el-tag :type="stc(row.status)">{{ row.status }}</el-tag></template></el-table-column><el-table-column label="事由" prop="reason" min-width="120" show-overflow-tooltip/>
...
<script setup>
import { ref,onMounted } from 'vue';import { borrowApi } from '@/api/borrow';import { ElMessage } from 'element-plus'
const loading=ref(false);const list=ref([]);const page=ref(1);const size=ref(20);const total=ref(0);const activeTab=ref('')
