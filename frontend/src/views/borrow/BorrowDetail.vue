<template>
  <div class="detail" v-loading="loading"><el-page-header @back="$router.back()" content="借用详情" style="margin-bottom:15px"/>
    <el-card v-if="record"><el-descriptions :column="2" border><el-descriptions-item label="借用单ID">{{ record.id }}</el-descriptions-item><el-descriptions-item label="设备ID">{{ record.deviceId }}</el-descriptions-item><el-descriptions-item label="开始时间">{{ fmt(record.startTime) }}</el-descriptions-item><el-descriptions-item label="结束时间">{{ fmt(record.endTime) }}</el-descriptions-item><el-descriptions-item label="状态"><el-tag :type="stc(record.status)">{{ stName(record.status) }}</el-tag></el-descriptions-item><el-descriptions-item label="当前步骤">第{{ record.currentStep }}步</el-descriptions-item><el-descriptions-item label="事由" :span="2">{{ record.reason }}</el-descriptions-item><el-descriptions-item label="实际归还" v-if="record.realReturnTime">{{ fmt(record.realReturnTime) }}</el-descriptions-item><el-descriptions-item label="逾期天数" v-if="record.overdueDays">{{ record.overdueDays }}天</el-descriptions-item><el-descriptions-item label="损坏报告" :span="2" v-if="record.damageReport">{{ record.damageReport }}</el-descriptions-item></el-descriptions>
    </el-card>
