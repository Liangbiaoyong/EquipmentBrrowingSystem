    ...
    <el-tabs v-model="tab">
      <el-tab-pane label="实验室列表" name="labs">
        <el-card>
          ...
          <el-table :data="labs" stripe row-key="id" @sort-change="onLabSort">
            ...
            <el-table-column prop="id" label="ID" width="80" sortable="custom"/><el-table-column prop="name" label="实验室名称" min-width="220" sortable="custom"/><el-table-column prop="code" label="编码" width="140" sortable="custom"/>
            <el-table-column label="状态" width="80" sortable="custom" prop="status"><template #default="{row}"><el-tag :type="row.status===1?'success':'info'">{{ row.status===1?'启用':'禁用' }}</el-tag></template></el-table-column>
      ...
      <el-tab-pane label="地点映射" name="rooms">
        <el-card>
          ...
          <el-table :data="rooms" stripe @sort-change="onRoomSort">
            <el-table-column prop="id" label="ID" width="80" sortable="custom"/>
            <el-table-column label="所属实验室" width="200" sortable="custom" prop="laboratoryId"><template #default="{row}">{{ row.laboratoryId?labName(row.laboratoryId):'未分配' }}</template></el-table-column>
            <el-table-column prop="roomName" label="房间地址" width="140" sortable="custom"/>
            <el-table-column prop="fullLocation" label="完整位置" min-width="280" show-overflow-tooltip sortable="custom"/>
...
const labSortBy=ref('');const labSortOrder=ref('');const roomSortBy=ref('');const roomSortOrder=ref('')
...
function onLabSort({prop,order}){labSortBy.value=order?prop:'';labSortOrder.value=order==='ascending'?'asc':order==='descending'?'desc':'';loadLabs()}
function onRoomSort({prop,order}){roomSortBy.value=order?prop:'';roomSortOrder.value=order==='ascending'?'asc':order==='descending'?'desc':'';loadRooms()}
async function loadLabs(){try{const{data}=await axios.get('/laboratories',{params:{page:labPage.value,size:labSize.value,keyword:labKeyword.value,sortBy:labSortBy.value||undefined,order:labSortOrder.value||undefined}});labs.value=data.records||[];labTotal.value=data.total||0}catch{}}
async function loadAllLabs(){try{const{data}=await axios.get('/laboratories/list');allLabs.value=data||[]}catch{}}
async function loadRooms(){try{const{data}=await axios.get('/laboratories/rooms',{params:{page:roomPage.value,size:roomSize.value,laboratoryId:roomFilter.value,roomName:roomSearch.value,sortBy:roomSortBy.value||undefined,order:roomSortOrder.value||undefined}});rooms.value=data.records||[];roomTotal.value=data.total||0}catch{}}
