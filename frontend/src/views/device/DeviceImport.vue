<template>
  <div class="import"><h2>批量导入</h2>
    <el-card><el-upload drag :auto-upload="false" :on-change="onFileChange" accept=".csv,.xlsx" :limit="1"><el-icon :size="50"><UploadFilled/></el-icon><div>拖拽或点击上传 CSV / XLSX 文件</div></el-upload>
...
async function doDryRun(){if(!file.value)return;dryLoading.value=true;try{const{data}=await deviceApi.dryRun(file.value);result.value=data;ElMessage.success('预览完成')}catch{}finally{dryLoading.value=false}}
async function doImport(){if(!file.value)return;impLoading.value=true;try{const{data}=await deviceApi.importFile(file.value);result.value=data;ElMessage.success('导入完成');loadBatches()}catch{}finally{impLoading.value=false}}
