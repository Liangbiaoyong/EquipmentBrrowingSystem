<template>
  <div class="device-list"><h2>设备列表</h2>
    <el-card class="search-card"><el-row :gutter="10"><el-col :span="4"><el-input v-model="q.keyword" placeholder="名称/编号/型号" clearable @keyup.enter="search"/></el-col><el-col :span="3"><el-select v-model="q.categoryId" placeholder="分类" clearable @change="search"><el-option v-for="c in categories" :key="c.id" :label="c.name" :value="c.id"/></el-select></el-col><el-col :span="3"><el-select v-model="q.status" placeholder="状态" clearable @change="search"><el-option label="正常" :value="1"/><el-option label="维修中" :value="2"/><el-option label="报废" :value="3"/></el-select></el-col><el-col :span="3"><el-select v-model="q.statusType" placeholder="借还状态" clearable @change="searchByStatus"><el-option label="闲置" value="idle"/><el-option label="借用中" value="borrowing"/><el-option label="不可借" value="unavailable"/><el-option label="待维修" value="repair"/></el-select></el-col><el-col :span="3"><el-button type="primary" @click="search">搜索</el-button></el-col></el-row></el-card>
