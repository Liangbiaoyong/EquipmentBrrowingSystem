<template>
  <div class="device-list"><h2>设备列表</h2>
    <el-card class="search-card"><el-row :gutter="10"><el-col :span="4"><el-input v-model="q.keyword" placeholder="名称/编号/型号" clearable @keyup.enter="search"/></el-col><el-col :span="3"><el-select v-model="q.categoryId" placeholder="分类" clearable @change="search"><el-option v-for="c in categories" :key="c.id" :label="c.name" :value="c.id"/></el-select></el-col><el-col :span="3"><el-select v-model="q.status" placeholder="状态" clearable @change="search"><el-option label="可借用" :value="1"/><el-option label="借用中" :value="2"/><el-option label="维修中" :value="3"/><el-option label="待报废" :value="4"/></el-select></el-col><el-col :span="3"><el-select v-model="q.statusType" placeholder="借还状态" clearable @change="searchByStatus"><el-option label="闲置" value="idle"/><el-option label="借用中" value="borrowing"/><el-option label="不可借" value="unavailable"/><el-option label="待维修" value="repair"/></el-select></el-col><el-col :span="3"><el-button type="primary" @click="search">搜索</el-button></el-col></el-row></el-card>
    <el-card style="margin-top:15px">
      <el-table :data="list" v-loading="loading" stripe @row-click="toDetail" style="cursor:pointer"><el-table-column prop="assetNo" label="资产编号" width="140"/><el-table-column prop="name" label="名称" min-width="160"/><el-table-column prop="model" label="型号" min-width="120"/><el-table-column label="分类" width="120"><template #default="{row}">{{ catName(row.categoryId) }}</template></el-table-column><el-table-column label="国标分类" prop="gbCategoryName" width="130" show-overflow-tooltip /><el-table-column label="使用人" width="90" prop="custodian"/><el-table-column label="状态" width="90"><template #default="{row}"><el-tag :type="st(row.status)">{{ stx(row.status) }}</el-tag></template></el-table-column><el-table-column prop="location" label="存放地" min-width="130" show-overflow-tooltip/></el-table>
