  ...
  <div class="category"><h2>分类管理</h2>
    <el-tabs v-model="tab">
      ...
      <el-tab-pane label="映射规则" name="mappings">
        <el-card>
          <div style="margin-bottom:12px;display:flex;gap:10px"><el-select v-model="mappingFilter" placeholder="按分类筛选" clearable @change="loadMappings" style="width:200px"><el-option v-for="c in categories" :key="c.id" :label="c.name" :value="c.id"/></el-select><el-input v-model="testGbName" placeholder="国标分类名" style="width:200px"/><el-button @click="testClassify">测试分类</el-button><el-button type="primary" @click="mappingForm={};mappingVisible=true">+ 新增规则</el-button></div>
...
const mappingForm=ref({});const mappingVisible=ref(false);const testGbName=ref('');const classifyResult=ref('')
...
async function loadMappings(){try{const{data}=await axios.get('/categories/mappings',{params:{categoryId:mappingFilter.value}});mappings.value=data||[]}catch{}}
